"""
Weather Temperature Prediction Model - 1D CNN
Single-task model for 7-day temperature forecasting

Features:
- 11 input variables (excludes TG, PP, QQ)
  - Base features: TN, TX, RR, SS, HU, FG, FX, CC, SD, DAY_SIN, DAY_COS
- 7-day forecast horizon
- Year-based train/val/test split (1957-2022 / 2023 / 2024-2025)
- Architecture: 1D CNN with multiple conv layers
- Designed for local temporal patterns and cycles
...
print("\n[1/9] Loading data...")
# Load the clean data from dataManager (11 features, no TG)
...
target_data = clean_df[[TARGET_COL]].values
...
# Flatten target to (samples, 7) for CNN output
...
class WeatherCNN(nn.Module):
    """1D CNN for weather forecasting"""

    def __init__(self, input_features, sequence_length, forecast_days):
        super().__init__()

        # Conv1d expects (batch, channels, length)
        # Our input: (batch, sequence_length, features) -> transpose to (batch, features, sequence_length)

        # Multiple conv layers to capture patterns at different scales
        ...
        # sequence_length=30 -> pool(30)=15 -> pool(15)=7 -> pool(7)=3
        self.flattened_size = 128 * 3

        # Dense layers
        self.fc1 = nn.Linear(self.flattened_size, 256)
        self.fc2 = nn.Linear(256, 128)
        self.fc3 = nn.Linear(128, forecast_days)
...
# Instantiate model
model = WeatherCNN(N_FEATURES, INPUT_DAYS, FORECAST_DAYS).to(device)
...
print(f"  1D CNN with 3 conv layers (64, 128, 128 channels)")
print(f"  Kernel sizes: 3, 3, 5")
print(f"  MaxPooling after each conv layer")
print(f"  Dense layers: 256 -> 128 -> {FORECAST_DAYS}")
...
for epoch in range(N_EPOCHS):
    ...
    if avg_val_loss < best_val_loss:
        ...
        epochs_no_improve = 0
        torch.save(model.state_dict(), 'best_model_cnn_temperature.pth')
...
# Load best model
model.load_state_dict(torch.load('best_model_cnn_temperature.pth'))
model.eval()
...
actuals = y_scaler.inverse_transform(actuals_scaled)
...
day1_actual = actuals[:n_samples_to_plot, 0]
day1_pred = predictions[:n_samples_to_plot, 0]
...
day3_actual = actuals[:n_samples_to_plot, 2]
day3_pred = predictions[:n_samples_to_plot, 2]
...
day7_actual = actuals[:n_samples_to_plot, 6]
day7_pred = predictions[:n_samples_to_plot, 6]
...
residuals_day1 = actuals[:n_samples_to_plot, 0] - predictions[:n_samples_to_plot, 0]
residuals_day7 = actuals[:n_samples_to_plot, 6] - predictions[:n_samples_to_plot, 6]
...
plt.suptitle(f'Temperature Forecast Evaluation - CNN ({test_dates[0].strftime("%Y-%m-%d")} to {test_dates[n_samples_to_plot-1].strftime("%Y-%m-%d")})',
             fontsize=14, fontweight='bold', y=0.995)
plt.tight_layout()
plt.savefig('./figures/temperature_forecast_evaluation_cnn.png', dpi=800, bbox_inches='tight')
print(f"  Saved: ./figures/temperature_forecast_evaluation_cnn.png")
